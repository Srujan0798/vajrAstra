...
## 20. STATE AT 14 SEP (post-interrogation cycles 1-2 — the truth-hardening phase)
- INTERROGATION PROTOCOL V2 ACTIVE (research/INTERROGATION_PROTOCOL_V2.md + BOSS_AGENT_LOAD_ORDER.md): RED attacks / BLUE defends from disk / EXEC runs 4-page gates / SCRIBE logs; question fission (every answered P0/P1 spawns 1-3); hybrid human-loop (H1-H5 batch to operator once daily); comfort audit (zero conflicts exposed = FAILED day).
- GATES RUN (all 4-page law, all closed): B1 doctr PARSeq = NO (no Indic weights in 1.1.0, mojibake 0/4, fake-fidelity probe documented) · B3 tessdata_best = LOSS 0/12 (homebrew mal already best-by-md5; PSM6 helps only te_001) · B2 rapidocr server-rec = NO-SERVER-INDIC (every _rec_server weight is ch_*; te/ta/ka mobile-only — disk-proven from default_models.yaml) · B4 paddle server-det = NO-GAIN (full mode byte-identical to shipped 4/4 incl. te_053 historical-timeout page; timeouts were parallel-load transients; light fallback = insurance) · B12 ensemble voting = NO (0/115 beat best single; agreement-text CER 0.897 vs 0.628; union 3.92 worse than worst single on 115/115 — line-granularity mismatch). DECK LAW: consensus = confidence marker, never GT, never claimed as free pseudo-GT.
- GT-QUALITY GATE NOW IN THE WRITER (durable C2 fix, 14 Sep): verify_v2.py nulls CER/WER with reason=legacy_mojibake_layer on Indic-tagged pages whose PDF layer is <15% Indic chars (55 pages flagged; 550 CERs can never contaminate rankings through ANY regen; gt_thin nulls = 2190). Patching output files was tried twice and wiped by regen — one-writer law lesson: fix the WRITER, never the output.
- HONEST RANKING (writer basis, fresh every regen): surya 0.430 median CER > anuvaad 0.475 > tesseract-family 0.49 > IPO 0.63 > paddle 0.74 > rapidocr 0.75 > doctr 0.85 > easyocr 0.88. Two-tier story: {surya, anuvaad, tess-family} clear tier-1; doctr #1-by-wins was mojibake-GT artifact (fell to 8th-9th on clean basis). Per-script: surya wins Tamil+Devanagari decisively; te/kn all-engines 0.85-0.92 (hard scripts); ml small-n.
- BOOTSTRAP VERDICT (1000 paired resamples): surya-vs-anuvaad, surya-vs-IPO, anuvaad-vs-IPO ALL TIED on the 115-page basis (CIs straddle 0). Recompute on writer basis pending. Deck claim = "two tiers", never "surya wins".
- AKER vs CER (grapheme-cluster error rate, 20-page probe): reorders top-2 (AKER: surya < anuvaad; CER: anuvaad < surya) → debate C×G LIVE — full-basis AKER + DECISIONS verdict pending. Deck metric claims gated on this.
- CONTAMINATION INCIDENT (14 Sep, exposed+purged): stale pre-v2 verify_v2.py/report.py overwrote sealed HEAD post-commit; partial metrics re-run dirtied 10 engines' RUN.md; 4 scaffold .py appeared (1 with syntax error). Purged via git restore. LESSON (LAW): agents NEVER edit shared pipeline files (verify_v2/report/run_engine/seal_gen/report_gen) — agent work lives in research/gates/ + standalone generators only. External "improvement plans" audited against disk BEFORE execution: ~70% of their P0/P1 items were contamination phantoms (dup functions, missing CER) or already-done (render sha1, 300-dpi probe, regression alarm, consensus naming, GAP_ANALYSIS is legit one-writer artifact — rm would violate archive law).
- CODE CLEANUPS DONE: engines_config.py = single source of truth for ENGINES + ENGINE_VERSIONS (7 duplicate sites consolidated) · seal_gen.py ENGINE_LIMITATIONS (paddle ml=en-fallback with ml-ratio-0.0 disk proof; surya no-lang-hint 0.22.1; easyocr combos validated) now auto-written into every RUN.md + metrics.json · preprocess_image docstring truth-fixed (binarize-only, NO deskew — was a lie) · PREPROCESSING_UPLIFT_WHY.md (why -879: 17/20 pages base>0→bin=0; empties are contentless, binarization destroys faint strokes; keep retry-only, gate off expansion).
- QUARANTINE (operator H-queue): _quarantine/{level3_readiness.py, preprocessing_uplift.py, regression_test.py} — uninvited scaffolds, compile-clean, awaiting operator keep/kill. _archive/export_training_data_broken_20260914.py (syntax-error duplicate of research/training_assets_gen.py).
- TRAINING ASSETS (regen on writer basis): SFT 1760 lines / DPO 126 pairs / disagreement queue 50 pages. All read CER_STAGE3B nulls automatically (one-writer inheritance).
- REMAINING BEFORE SEP-16 PACKAGE: (a) C×G AKER verdict (full clean basis) → DECISIONS.log; (b) bootstrap CIs recomputed on writer basis (55-null) for the two-tier claim; (c) CER_BY_SCRIPT.md regen on writer basis; (d) SEPT16_PACKAGE.md + WHATSAPP 5-liner refresh with honest two-tier + gap numbers; (e) evening summary + comfort audit; (f) operator H-queue: quarantine keep/kill, P4 elite items (cascade router, distillation, Triton serving, active-learning UI) = post-Sep-16 by design.
